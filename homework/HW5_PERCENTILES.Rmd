...
In some cities in Maryland, property crimes happen at a lot higher rates than violent crime rates. However, some cities are inversed and have higher violent crime rates than property crime rates. 
...
```{r}
md_cities_change_violent <- md_cities_rates |>  mutate(pct_change = (violent_rate_2019-violent_rate_2020)/violent_rate_2019 *100)
```
```{r}
md_cities_change_property <- md_cities_rates |>  mutate(pct_change = (property_rate_2019-property_rate_2020)/property_rate_2019 * 100)
```
...
There are a few towns with negative infinity percent change and some with infinite percent change
...
The number of violent crimes are mostly decreasing in Maryland but the amount of reported property crimes are rising. 
...
```{r}
md_cities_percentiles <- md_cities_change_violent |> mutate(percentile = ntile(violent_rate_2020, 100)) 
```
```{r}
IQR_2020_violentcrime <- md_cities_percentiles |> summarize(iqr_value= IQR(percentiles)
```
