...
# Read the data
car_thefts <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/car_thefts_month.csv")  |> arrange(month)  # Sort by date
...
The numbers of total car thefts are decreasing over time; however, we do not know what it is per capita. 
...
It shows how the data compares to the previous mean; therefore, showing a change of decreasing.
...
- Most of the data lies in the IQR; however, using the mean high and low outliers can center the mean somewhere else. 
- The second one helps show it in perspective of the previous mean rather than oh, car thefts are decreasing. The mean gives something the viewer to reference. 
- Car thefts decrease going into 2025. 
...
period_summary <- car_thefts |> 
  ...
  summarize(
    ...
    months = n(),
    se_thefts = sd_thefts / sqrt(months),
    margin_error = qt(0.975, df = 45.26182 - 1) * se_thefts, # look at how we calculated this above
    ci_lower = mean_thefts - margin_error,
    ci_upper = mean_thefts + margin_error
  )
...
The hypthoesis was proven; therefore, there is a true decrease across the two periods. I would revise the headline to have new data says. 
