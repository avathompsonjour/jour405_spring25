...
```{r}
carthefts <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/car_thefts_jan24.csv")
```

### Task 3: Calculate the standard deviation of the number of car thefts, along with the mean
```{r}
carthefts |> summarize(mean = mean(count), sd = sd(count))
```
...
carthefts |> 
  ...
  geom_vline(aes(xintercept = sd(count)), color = "blue" ,linetype = "dashed" , size=1) +
...
The distribution is skewed to the right, meaning more cars between 12 and 17 are getting stolen. 
